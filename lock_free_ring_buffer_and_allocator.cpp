...
class LockFreeBoundedBuffer {
    ...
    public:
    LockFreeBoundedBuffer(int size, int num_threads) {
        capacity = size;
        numThreads = num_threads;
        allocator = new T*[numThreads];
        allocatorHead = new int[numThreads]();
        for(int i = 0; i < num_threads; i++) {
            allocator[i] = new T[size];
        }
    ...
    void add (T item, int tid) {
        sem_wait(&slots);
        int index = allocatorHead[tid];
        AtomicPtr newReference;
        newReference.ptr = &(allocator[tid][index]);
        *(newReference.ptr) = item;
        allocatorHead[tid] = (index + 1) % capacity;
...
LockFreeBoundedBuffer<int> temp_buf(10,100);
long t_array[100];
